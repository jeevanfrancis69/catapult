...
def show_no_target(oled):
    """Displayed when scan completes but no targets were detected."""
    oled.fill(0)
    _big_text(oled, "No",     0, 0,  scale=2)
    _big_text(oled, "Target", 0, 18, scale=2)
    _big_text(oled, "Found",  0, 36, scale=2)
    oled.show()
...
def show_target_found(oled, distance_mm):
    ...
    _big_text(oled, "Acquired", 0, 18, scale=2)
...
def show_firing(oled):
    ...
    oled.fill(0)
    _big_text(oled, "Firing", 0, 24, scale=2)
    oled.show()

def show_target_destroyed(oled):
    """Displayed after the target is confirmed gone."""
    oled.fill(0)
    _big_text(oled, "Target",    0, 0,  scale=2)
